...
def get_system_prompt():
    ...
    system_prompt = f"""你是一个聊天助手，具有搜索聊天历史、查询AnythingLLM知识库、文件操作和使用技能的能力。
...
3. 如果用户需要读取文件，调用read_file工具
4. 如果用户需要写入文件，调用write_file工具
5. 如果用户需要删除文件，调用delete_file工具
6. 如果用户需要列出目录，调用list_files工具
7. 如果用户的请求需要使用某个技能，调用use_skill工具，并指定技能名称
8. 调用工具时必须使用<function_calls>标签包裹
9. 参数必须是JSON格式
10. 如果不需要调用工具，可以直接回答问题"""
...
def main():
    ...
    print("[系统] 正在加载技能列表...")
    skills = list_available_skills()
    print(f"[系统] 技能加载完成，共 {len(skills)} 个技能")
            ...
            if response:
                ...
                if func_call:
                    ...
                    if func_call['name'] == 'anythingllm_query':
                        ...
                        print(f"\n工具执行结果:\n{result}")
                        history.append({"role": "assistant", "content": response + "\n\n工具执行结果:\n" + result})
                    elif func_call['name'] == 'use_skill':
                        skill_name = func_call['arguments'].get('skill_name', '')
                        ...
                        if skill_response:
                            ...
                            history.append({"role": "assistant", "content": response + "\n\n技能执行结果:\n" + skill_response})
                            if skill_usage:
                                total_prompt_tokens += skill_usage.get('prompt_tokens', 0)
                                total_completion_tokens += skill_usage.get('completion_tokens', 0)
                        else:
                            print("\n技能执行失败")
                            history.append({"role": "assistant", "content": response + "\n\n技能执行失败"})
                    else:
